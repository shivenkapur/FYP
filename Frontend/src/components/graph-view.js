...
class GraphView extends React.Component{
  ...
  addNewEdges(edges, oldEdgesMap, selectedEdge, prevSelectedEdge, forceRender = false) {
    if (!this.state.draggingEdge) {
      let edge = null;
  ...
  handleWrapperKeydown = d => {
    ...
    switch (d.key) {
      ...
      case 'Backspace':
        ...
        if (selectedNodeObj && !selectedNodeObj.node) {
      ...
      <div className="view-wrapper" ref={this.viewWrapper}>
        <svg className="graph" ref={this.graphSvg}>
          ...
          <g className="view" ref={el => (this.view = el)}>
            ...
            <g requiredFeatures="http://www.w3.org/Graphics/SVG/feature/1.2/#TextFlow" className="entities" ref={el => (this.entities = el)} />
          </g>
        </svg>
        <div
